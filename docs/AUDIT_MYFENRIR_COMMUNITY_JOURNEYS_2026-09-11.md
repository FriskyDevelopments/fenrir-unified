...
**P0 (accepted 2026-09-11):** ship DNS proof with **legacy re-verify + operator/tenant comms** (no backfill). UI banners + docs ops blurb required before prod flip.
...
| --- | ---: | --- |
| **P0** | **1** | DNS legacy-domain public routing break — **accepted**: re-verify + operator comms (no keep-last-known-good) |
...
- Operator communications + visible DnsWizard / DnsLookupPanel re-verify banners (EN+ES; FR/DE retained).

### Deferred / decided
- **Decided:** accept recheck behavior that can drop public routing (no keep-last-known-good).
- **Decided:** forced re-verify + operator/tenant comms for domains lacking proof fields (no migration/backfill in this PR).
...
## Frisky OK recorded (2026-09-11)

1. **Ship DNS with legacy re-verify + operator comms** — legacy domains go dark until Prepare → Verify; no backfill in this PR.
2. **Do not** change recheck to keep-last-known-good — temporary routing loss on recheck is accepted.

## What still needs Frisky OK

1. Whether `App.tsx` remains the long-term `/main` entry after IA rebuild (recommended short-term: yes; long-term: thin App → DashboardRoute).
2. Whether community primary should stay gold globally (this PR shifts it) vs brand-themed red for non-Fenrir tenants.
