...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  await context.env.DB.prepare(
    `UPDATE frisky_live_rooms SET status = 'paused' WHERE id = ? AND org_id = ?`
  )
    .bind(room.id, session.frisky_org_id)
    .run();
  await addAudit(context.env.DB, session, 'live_room_paused', 'FriskyLiveRoom', room.id, {
    slug: room.slug,
  });
