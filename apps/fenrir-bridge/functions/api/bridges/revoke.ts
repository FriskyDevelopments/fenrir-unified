...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  await context.env.DB.batch([
    ...
    context.env.DB.prepare(
      `UPDATE frisky_bridges SET status = 'revoked', revoked_at = ? WHERE id = ? AND org_id = ?`
    ).bind(ts, bridge.id, session.frisky_org_id),
  ]);
  await addAudit(context.env.DB, session, 'bridge_revoked', 'FriskyBridge', bridge.id, {
    slug: bridge.slug,
  });

  const row = await context.env.DB.prepare(`SELECT * FROM frisky_bridges WHERE id = ?`)
    .bind(bridge.id)
    .first<any>();
