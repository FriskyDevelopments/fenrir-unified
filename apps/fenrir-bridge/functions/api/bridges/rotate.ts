...
import { addAudit, createProductId, getBridgeForOrg, mapBridge, mapInvite } from "../../_lib/product-db";
...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  await context.env.DB.batch([
    ...
    context.env.DB
      .prepare(`INSERT INTO frisky_invites (id, bridge_id, invite_link, status, created_at) VALUES (?, ?, ?, 'active', ?)`)
      .bind(inviteId, bridge.id, `https://t.me/+pending-${inviteId}`, ts),
    context.env.DB
      .prepare(`UPDATE frisky_bridges SET current_invite_id = ?, status = 'active', rotated_at = ?, revoked_at = NULL WHERE id = ? AND org_id = ?`)
      .bind(inviteId, ts, bridge.id, session.frisky_org_id)
  ]);

  await addAudit(context.env.DB, session, "invite_rotated", "FriskyBridge", bridge.id, { publicUrl: bridge.publicUrl });
  const [updated, invite] = await Promise.all([
    context.env.DB.prepare(`SELECT * FROM frisky_bridges WHERE id = ?`).bind(bridge.id).first<any>(),
