...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  const body = await context.request
    .json<{ bridgeId?: unknown; chatId?: unknown }>()
    .catch(() => null);
  const bridgeId = sanitizeBridgeId(body?.bridgeId);
  const chatId = sanitizeChatId(body?.chatId);
  if (!bridgeId && !chatId)
    return noStoreJson({ ok: false, error: 'telegram_bridge_required' }, { status: 400 });
  if (chatId && !/^-?\d{5,}$/.test(chatId))
    return noStoreJson({ ok: false, error: 'invalid_telegram_chat_id' }, { status: 400 });
    ...
    return noStoreJson(
      {
        ...
        detail:
          'Recovery invites can only be created for an active Telegram lock in your Fenrir workspace.',
      },
      { status: 404 }
    );
  }
