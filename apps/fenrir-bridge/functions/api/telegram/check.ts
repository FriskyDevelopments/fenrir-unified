...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  await context.env.DB
    ...
    .bind(session.frisky_org_id, chatId, good ? 1 : 0, good ? 1 : 0, good ? 1 : 0, check.status, new Date().toISOString())
    .run();

  if (!good) {
    await addAudit(context.env.DB, session, "telegram_permission_failed", "TelegramChat", chatId, { chatId });
  }
