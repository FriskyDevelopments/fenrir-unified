...
async function handleMessage(
  ...
  if (linkCode) {
    ...
    const canonical = accountLinksConfigured(env)
    ...
    const result =
      canonical ??
      (await consumeTelegramAccountLinkCode(env, env.DB!, linkCode, claim));
    if (canonical?.ok) {
      await consumeTelegramAccountLinkCode(env, env.DB!, linkCode, claim).catch(
        (error) => console.error("telegram_d1_mirror_failed", String(error))
      );
    }
    await telegramApi(
      env,
        ...
        reply_markup: result.ok
          ? {
              inline_keyboard: [
                [
                  {
                    text: "Continue Community Gate",
                    url: COMMUNITY_BRIDGE_CONTINUE_URL,
                  },
                ],
              ],
            }
          : {
              inline_keyboard: [
                [
                  {
                    text: "Link FriskyDev ID",
                    url: FRISKY_TELEGRAM_LINK_START,
                  },
                ],
              ],
            },
...
const FRISKY_TELEGRAM_LINK_START =
  "https://www.myfenrir.com/api/telegram/link/start";
const COMMUNITY_BRIDGE_CONTINUE_URL =
