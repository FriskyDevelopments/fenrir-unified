...
import { addAudit, cleanSlug, createProductId, getDomainForOrg, mapBridge, mapInvite, publicUrl } from "../_lib/product-db";
...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  if (!chatId) return noStoreJson({ ok: false, error: "telegram_chat_id_required" }, { status: 400 });
  ...
  const groupName = typeof body?.telegramGroupName === "string" && body.telegramGroupName.trim()
    ? body.telegramGroupName.trim()
    : `${slug} Telegram Group`;
  const groupImageUrl = typeof body?.telegramGroupImageUrl === "string" ? body.telegramGroupImageUrl.trim() : "";
  ...
  await context.env.DB.batch([
    ...
    context.env.DB
      ...
      .bind(bridgeId, session.frisky_org_id, domain.id, slug, url, chatId, groupName, groupImageUrl, inviteId, ts, ts)
  ]);

  await addAudit(context.env.DB, session, "bridge_created", "FriskyBridge", bridgeId, { slug, telegramChatId: chatId });
