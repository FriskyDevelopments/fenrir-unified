...
export async function onRequestPost(context: { request: Request; env: BillingEnv & { RECONCILE_SHARED_SECRET?: string } }) {
  ...
  const ok = await verifySignature(secret, rawBody, context.request.headers.get("X-Reconcile-Signature"));
  if (!ok) return Response.json({ ok: false, error: "bad_signature" }, { status: 401 });

  let ids: string[];
  try {
    const parsed = JSON.parse(rawBody) as { event_ids?: unknown };
    ids = Array.isArray(parsed.event_ids) ? parsed.event_ids.filter((x): x is string => typeof x === "string") : [];
  } catch {
  ...
  for (const id of ids) {
    if (!(await tryClaimStripeEvent(context.env.DB, id))) { result.skipped.push(id); continue; }
    try {
      const event = await stripe.events.retrieve(id); // authoritative — caller payload never trusted
      if (!RELEVANT.has(event.type)) { result.skipped.push(id); continue; }
      const subId = subscriptionIdFromEvent(event);
      if (!subId) { result.skipped.push(id); continue; }
