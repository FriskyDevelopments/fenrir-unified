...
export async function onRequestPost(context: any) {
  ...
  if (!body?.accessToken) return noStoreJson({ ok: false, error: "missing_supabase_access_token" }, { status: 400 });
  try {
    ...
    return noStoreJson({ ok: true }, { headers: { "Set-Cookie": sessionSetCookie(token) } });
  } catch (error) {
