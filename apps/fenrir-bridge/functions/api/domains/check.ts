...
export const onRequestPost: PagesFunction<BillingEnv> = async (context) => {
  ...
  if (!domain) return noStoreJson({ ok: false, error: "domain_not_found", message: "Domain not found." }, { status: 404 });

  const verified = await verifyDomainTxtRecord(domain.txtRecordName, domain.txtRecordValue);
  if (!verified) {
    await context.env.DB
      .prepare(`UPDATE frisky_domains SET status = 'failed' WHERE id = ? AND org_id = ?`)
      .bind(domain.id, session.frisky_org_id)
      .run();
    await addAudit(context.env.DB, session, "dns_check_failed", "FriskyDomain", domain.id, { domain: domain.domain });
    return noStoreJson({ ok: false, error: "dns_txt_not_verified", message: "Required Fenrir TXT ownership record was not found." }, { status: 400 });
  }

  const verifiedAt = new Date().toISOString();
  await context.env.DB
    .prepare(`UPDATE frisky_domains SET status = 'verified', certificate_status = 'active', verified_at = ? WHERE id = ? AND org_id = ?`)
    .bind(verifiedAt, domain.id, session.frisky_org_id)
    .run();
  await addAudit(context.env.DB, session, "domain_verified", "FriskyDomain", domain.id, { domain: domain.domain });

  const row = await context.env.DB.prepare(`SELECT * FROM frisky_domains WHERE id = ?`).bind(domain.id).first<any>();
