...
export async function buildAuthorizationUrl(
  ...
  const enabled = new Set(
    (env.WORKOS_DIRECT_CONNECTIONS ?? 'google,microsoft,apple')
      .split(',')
      .map((s) => s.trim().toLowerCase())
      .filter(Boolean)
  );
  const canJump =
    Boolean(providerHint) &&
    env.WORKOS_ALLOW_PROVIDER_HINT === '1' &&
    enabled.has(providerHint as string);
...
export async function readState(
  request: Request,
  env: WorkOSEnv
): Promise<WorkOSStatePayload | null> {
