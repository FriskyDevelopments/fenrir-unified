  ...
  return noStoreJson(
    {
      ...
      detail: {
        message:
          'Community Gate member auth uses Firebase Auth. Set FIREBASE_PROJECT_ID for token verification and NEON_DATABASE_URL for the gate data plane, then apply docs/neon-community-gate-schema.sql in Neon.',
        missing,
      },
    },
    { status: 503 }
...
export async function requireCommunityGateUser(
  request: Request,
  env: CommunityGateEnv
): Promise<CommunityGateUser> {
...
export async function assertCommunityStaff(
  env: CommunityGateEnv,
  user: CommunityGateUser,
...
async function readFirebaseBearerUser(
  request: Request,
  env: CommunityGateEnv
): Promise<CommunityGateUser | null> {
...
async function verifyFirebaseIdToken(
  ...
  if (payload.iss !== `https://securetoken.google.com/${projectId}`)
    throw new Error('firebase_token_invalid_issuer');
  if (!payload.sub || typeof payload.sub !== 'string')
    throw new Error('firebase_token_missing_subject');
    ...
    return noStoreJson(
      {
        ...
        detail:
          'Apply docs/neon-community-auth-schema.sql and docs/neon-community-gate-schema.sql in Neon.',
      },
      { status: 503 }
    );
  }
