...
export async function finalizeCommunityOAuthSignIn(options: {
  ...
  const { user, membership } = await ensureCommunityMembershipForEmail(
    sql,
    email,
    brand.communityOrgId
  );
  ...
  const cookie = communitySessionSetCookie(
    session,
    communityCookieDomain(options.request, options.env)
  );
...
async function upsertCommunityOAuthIdentity(
  sql: CommunitySql,
  userId: string,
  identity: OAuthIdentity
) {
  ...
  const providerSubject =
    separator >= 0 ? identity.identityId.slice(separator + 1) : identity.identityId;
  await sql`
