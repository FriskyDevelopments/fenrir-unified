...
export async function upsertProfileForSession(env: BillingEnv, session: SessionPayload, supabaseUserId: string) {
...
export async function patchProfileByUserId(env: BillingEnv, supabaseUserId: string, patch: ProfilePatch) {
  if (!serviceConfigured(env)) return { ok: false as const, error: "supabase_service_not_configured" };
  const url = env.SUPABASE_URL!.replace(/\/$/, "");
  ...
  const response = await fetch(`${url}/rest/v1/profiles?id=eq.${encodeURIComponent(supabaseUserId)}`, {
    method: "PATCH",
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY!,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
