...
test("a banned broker is not a community provider", () => {
  ...
  assert.equal(isCommunityOAuthProvider("authkit"), false);
...
test("start rejects a banned broker, a bad slug, and an unconfigured provider", async () => {
  const env = testEnv();

  const bannedBroker = await handleCommunityOAuthStart({
    request: new Request(`${ORIGIN}/api/community-auth/oauth/authkit?slug=fenrir`),
    env,
    provider: "bannedBroker"
  });
  assert.equal(bannedBroker.status, 404);
...
test("callback rejects bannedBroker outright", async () => {
  const response = await handleCommunityOAuthCallback({
    request: new Request(`${ORIGIN}/api/community-auth/oauth/callback/bannedBroker?code=abc`),
    env: testEnv(),
    provider: "bannedBroker"
  });
