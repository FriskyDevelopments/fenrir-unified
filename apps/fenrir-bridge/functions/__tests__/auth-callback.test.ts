...
describe('Direct Google OAuth callback (needs Google network + Supabase, now mocked)', () => {
  ...
  it('verifies the id_token, upserts the Supabase profile, and redirects to session transfer', async () => {
    ...
    const router = buildFetchRouter([
      ...
      {
        when: 'proj.supabase.co/rest/v1/profiles',
        respond: () => jsonResponse([{ id: 'google-sub-123' }]),
      },
    ...
    expect(response.headers.get('Location')).toContain(
      'https://app.example.test/api/auth/complete?token='
    );
    ...
    const supabaseCall = router.calls.find((c) =>
      c.url.includes('proj.supabase.co/rest/v1/profiles')
    );
    ...
    const transfer = JSON.parse(
      new TextDecoder().decode(
        Uint8Array.from(atob(encodedBody.replace(/-/g, '+').replace(/_/g, '/')), (c) =>
          c.charCodeAt(0)
        )
      )
  ...
  it('does not block login when the Supabase admin account API is unreachable', async () => {
    ...
    const router = buildFetchRouter([
      ...
      {
        when: 'proj.supabase.co/rest/v1/profiles',
        respond: () => jsonResponse([{ id: 'google-sub-err' }]),
      },
  ...
  it("returns 410 when the provider's direct OAuth credentials are not configured", async () => {
    ...
    const unconfigured: OAuthEnv = {
      ...env,
      GOOGLE_CLIENT_ID: undefined,
      GOOGLE_CLIENT_SECRET: undefined,
    };
    const request = new Request(
      'https://auth.example.test/api/auth/callback/google?code=x&state=y'
    );

    const response = await call(directCallback, {
      request,
      env: unconfigured,
