...
// MyFenrir sign-in is Supabase Auth, started client-side. These suites pin the
// retired server-side auth surface down: the legacy login route only forwards
// to /login, and the retired callbacks never mint a session — no matter how
// configured the environment looks.
...
describe("Retired server-side auth surface (Supabase-only sign-in)", () => {
  ...
  for (const provider of ["google", "microsoft", "apple"] as const) {
    it(`sends /api/auth/login/${provider} to the /login screen, never to a broker`, async () => {
      ...
      // No transaction cookie may be issued any more.
      expect(setCookies(response)).toHaveLength(0);
      expect(router.calls).toHaveLength(0);
    });
  }

  it("404s any unknown login provider (banned brokers included)", async () => {
    const request = new Request("https://auth.example.test/api/auth/login/other");
    const response = await call(directLogin, { request, env, params: { provider: "other" } });
