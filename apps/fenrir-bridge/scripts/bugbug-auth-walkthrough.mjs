...
const preview =
  process.env.BUGBUG_REUSE_SERVER === '1' ? null : await startStaticServer(Number(appPort));
const browser = spawn(
  chrome,
  [
    '--headless=new',
...
try {
  ...
  await step('front-door loads with Apple authorization action', async () => {
    ...
    `,
      'Continue with Apple button not found',
      15000
    );
  ...
  await step('Apple authorization begins and uses Supabase callback broker', async () => {
    ...
    const url = await waitForUrl(
      (next) =>
        next.hostname.includes('appleid.apple.com') || next.hostname.includes('supabase.co'),
      'authorization did not leave MyFenrir'
    );
    const parsed = new URL(url);
    const redirectUri = parsed.searchParams.get('redirect_uri') || '';
    if (!parsed.hostname.includes('appleid.apple.com'))
      throw new Error(`expected Apple authorize URL, saw ${url}`);
  ...
  await step('SPA callback route returns root login to /main', async () => {
    ...
    await navigate(
      `${appUrl}/auth/callback?error=access_denied&error_description=bugbug-spa-callback`
    );
    const url = await waitForUrl(
      (next) => next.pathname === '/main',
      'SPA callback did not land on /main'
    );
    return {
      url,
  ...
  await step('production hash callback returns root login to /main', async () => {
    ...
    const url = await waitForUrl(
      (next) => next.pathname === '/main',
      'hash callback did not land on /main'
    );
    return {
      url,
  ...
  await step('callback preserves an in-app destination like /locks', async () => {
    ...
    await navigate(
      `${appUrl}/auth/callback?error=access_denied&error_description=bugbug-locks-callback`
    );
    const url = await waitForUrl(
      (next) => next.pathname === '/locks',
      'callback did not return to /locks'
    );
    return {
      url,
  ...
  await step('unsafe callback destination falls back to /main', async () => {
    await navigate(`${appUrl}/`);
    await evaluate(
      "window.localStorage.setItem('fenrir_post_auth_destination', 'https://evil.example/path')"
    );
    await navigate(
      `${appUrl}/auth/callback?error=access_denied&error_description=bugbug-unsafe-destination`
    );
    const url = await waitForUrl(
      (next) => next.pathname === '/main',
      'unsafe destination did not fall back to /main'
    );
    return {
      url,
...
async function startStaticServer(port) {
  const server = createServer(async (request, response) => {
    ...
    const relativePath =
      pathname === '/' || !pathname.includes('.') ? 'index.html' : pathname.replace(/^\/+/, '');
...
async function waitForPageWebSocketUrl() {
  ...
  while (Date.now() - started < 10000) {
    try {
      ...
      const appPage = json.find(
        (target) =>
          target.type === 'page' && target.url?.startsWith(appUrl) && target.webSocketDebuggerUrl
      );
