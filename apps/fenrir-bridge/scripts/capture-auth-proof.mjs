...
try {
  ...
  await capture(1, "Start at public MyFenrir root before login", "Old behavior felt like a loop back to this page.");
  ...
  await capture(3, "Callback processed after root login", "Proof: route is /main, not / and not /auth/callback.");
  ...
  await waitForPath("/main");
  await capture(4, "Production-style root hash callback processed", "Proof: /#access_token-style callbacks leave / and land on /main.");

  await navigate(`${appUrl}/locks`);
  await evaluate("window.localStorage.setItem('fenrir_post_auth_destination', '/locks')");
  await capture(5, "Start from app route /locks before login", "Expected callback destination: /locks");

  await navigate(`${appUrl}/auth/callback?error=access_denied&error_description=proof`);
  await waitForPath("/locks");
  await capture(6, "Callback processed after /locks login", "Proof: route is /locks, preserving the app route.");
