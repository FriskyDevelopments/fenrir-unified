...
const directOauthGuardSource = readFileSync(
  join(root, 'workers/fenrir-direct-oauth-guard.js'),
  'utf8'
);
const directOauthSource = readFileSync(join(root, 'functions/_lib/oauth.ts'), 'utf8');
const directOauthLoginSource = readFileSync(
  join(root, 'functions/api/auth/login/[provider].ts'),
  'utf8'
);
const directOauthCallbackSource = readFileSync(
  join(root, 'functions/api/auth/callback/[provider].ts'),
  'utf8'
);
...
const workosCallbackSource = readFileSync(
  join(root, 'functions/api/auth/callback/workos.ts'),
  'utf8'
);
...
const checks = [
  {
    ...
    pass:
      apiSource.includes('VITE_DIRECT_AUTH_ORIGIN') &&
  ...
  },
  {
    ...
    pass:
      !apiSource.includes('supabaseAuth') &&
  ...
  },
  {
    ...
    pass:
      workosLoginSource.includes('auth_error: "workos_login_init_failed"') &&
  ...
  },
  {
    ...
    pass:
      workosCallbackSource.includes('readState') &&
  ...
  },
  {
    ...
    pass:
      directOauthLoginSource.includes('createOAuthTransaction(provider') &&
  ...
  },
  {
    ...
    pass:
      directOauthCallbackSource.includes('readOAuthTransaction') &&
  ...
  },
  {
    ...
    pass:
      appSource.includes('const managedDashboardPath = "/main";') &&
  ...
  },
  {
    ...
    pass:
      !directOauthGuardSource.includes('url.pathname === "/api/auth/me"') &&
  ...
  },
  {
    ...
    pass:
      mainSource.includes('installHoneybadgerBrowserReporter();') &&
  ...
  },
  {
    ...
    pass:
      workosSource.includes('value.startsWith("//")') &&
  ...
  },
  {
    ...
    pass:
      workosSource.includes('HttpOnly; Secure; SameSite=Lax') &&
...
function callbackDestinationPath(pathname, hasCallbackParams = false) {
  ...
  if (hasCallbackParams && (pathname === '/' || pathname === '/login'))
...
const cases = [
  ...
  {
    saved: 'https://evil.example/path',
    callback: '/auth/callback',
    hasCallbackParams: false,
    expected: '/main',
  },
  {
    saved: '//evil.example/path',
    callback: '/auth/callback',
    hasCallbackParams: false,
    expected: '/main',
  },
...
for (const testCase of cases) {
  ...
  if (actual !== testCase.expected) {
    failed = true;
    console.error(
      `not ok - ${testCase.saved} via ${testCase.callback} redirected to ${actual}, expected ${testCase.expected}`
    );
  } else {
    console.log(
      `ok - ${testCase.saved} via ${testCase.callback} redirects to ${testCase.expected}`
    );
  }
}
