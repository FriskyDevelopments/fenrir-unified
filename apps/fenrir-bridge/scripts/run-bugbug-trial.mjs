...
async function main() {
  ...
  if (!env.BUGBUG_API_TOKEN) {
    ...
    await writeFile(join(artifactsDir, "latest-frisky-mcp-packet.json"), `${JSON.stringify(packet, null, 2)}\n`);
  ...
  for (const suite of configuredSuites) {
    const slug = suite.product.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  ...
  const packet = {
    ...
    source: "bugbug",
    trialMode: true,
    policy: "Run P0/P1 smoke suites first during 14-day trial; feed failures to Frisky MCP for repo-specific fixes.",
    summary: {
      total: results.length,
      passed: results.filter((r) => r.status === "passed" && r.exitCode === 0).length,
      failed: results.filter((r) => r.exitCode !== 0 || ["failed", "error"].includes(r.status)).length,
      unknown: results.filter((r) => r.exitCode === 0 && !["passed", "failed", "error"].includes(r.status)).length
    },
    results
  };
  ...
  await writeFile(join(artifactsDir, "latest-frisky-mcp-response.json"), `${JSON.stringify(mcp, null, 2)}\n`);
  console.error(`BugBug packet saved: ${join(artifactsDir, "latest-frisky-mcp-packet.json")}`);
  console.error(mcp.ok ? "Frisky MCP feed: sent" : `Frisky MCP feed: skipped/failed (${mcp.reason || mcp.status})`);
