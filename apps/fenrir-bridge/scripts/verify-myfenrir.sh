...
for base in "${HOSTS[@]}"; do
  code="$(curl -sS -o /dev/null -w "%{http_code}" --max-time 20 "${base}/healthz" || echo "000")"
  if [[ "$code" != "200" ]]; then
    echo "FAIL ${base}/healthz HTTP ${code} (expected 200; 522 = origin timeout)"
    FAIL=1
  else
    body="$(curl -sS --max-time 20 "${base}/healthz")"
    if [[ "$body" != "ok" ]]; then
  ...
  callback_response="$(curl -sS --max-time 20 -w $'\n%{http_code}' "${base}/auth/callback" || printf '\n000')"
  ...
  auth_body="$(curl -sS --max-time 20 "${base}/api/auth/me" || true)"
  ...
  direct_oauth_response="$(curl -sS --max-time 20 -w $'\n%{http_code}' "${base}/api/auth/callback/apple" || printf '\n000')"
  direct_oauth_code="$(printf "%s" "$direct_oauth_response" | tail -n 1)"
  direct_oauth_body="$(printf "%s" "$direct_oauth_response" | sed '$d')"
  if [[ "$direct_oauth_code" != "410" || "$direct_oauth_body" != *'"direct_oauth_disabled"'* ]]; then
    echo "FAIL ${base}/api/auth/callback/apple HTTP ${direct_oauth_code}; expected 410 direct_oauth_disabled"
    FAIL=1
  else
    echo "OK   ${base}/api/auth/callback/apple"
  fi

  readiness_response="$(curl -sS --max-time 20 -w $'\n%{http_code}' "${base}/api/readiness" || printf '\n000')"
