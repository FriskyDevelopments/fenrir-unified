...
const tools = [
  {
    ...
    description:
      'Return the beta MCP/API server health payload and binding visibility without exposing secrets.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  },
  {
    ...
    description:
      'Return beta readiness signals for API routing, D1 binding, auth config, and payment config.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
...
export default {
  async fetch(request, env) {
    ...
    return json(
      { ok: true, service: SERVER_NAME, mcp: '/mcp', health: '/api/health' },
      {},
      request,
      env
    );
...
async function handleMcp(request, env, url) {
  if (request.method === 'GET') {
    if ((request.headers.get('accept') || '').includes('text/event-stream')) {
      return json(
        { ok: false, error: 'sse_not_enabled', endpoint: url.pathname },
        { status: 405 },
        request,
        env
      );
    ...
    return json(
      { ok: true, service: SERVER_NAME, endpoint: url.pathname, transport: 'streamable-http' },
      {},
      request,
      env
    );
...
function handleRestGet(request, env, url) {
  ...
  if (
    url.pathname === '/api/app-state' ||
    url.pathname === '/api/billing/status' ||
    url.pathname === '/api/telegram/link'
  ) {
    return json(
      { ok: false, error: 'authentication_required', source: SERVER_NAME },
      { status: 401 },
      request,
      env
    );
  ...
  if (
    url.pathname.startsWith('/api/public/bridge/') ||
    url.pathname.startsWith('/api/public/room/')
  ) {
    return json(
      { ok: false, error: 'public_route_not_configured', source: SERVER_NAME },
      { status: 404 },
      request,
      env
    );
  ...
  if (url.pathname.startsWith('/api/')) {
    return json(
      { ok: false, error: 'api_route_not_found', path: url.pathname, source: SERVER_NAME },
      { status: 404 },
      request,
      env
    );
...
function initializeResult(params = {}) {
  ...
  return {
    ...
    instructions:
      'Fenrir Bridge beta MCP server. Use read-only tools first. API routing remains isolated from the React SPA.',
  };
}
...
async function callTool(params = {}, env, url) {
  ...
  if (name === 'fenrir.route_audit')
    return toolResult(routeAuditPayload(args.path || '/api/health'));
...
function routeAuditPayload(pathValue) {
  ...
  const known =
    KNOWN_JSON_ROUTES.has(path) ||
    path.startsWith('/api/public/bridge/') ||
    path.startsWith('/api/public/room/');
  ...
  return {
    ...
    notes:
      isApi && !known
        ? 'Unknown API routes must return JSON 404, not index.html.'
        : 'Route is covered by the beta contract.',
  };
}
...
async function mcpAuthFailure(request, env, url) {
    ...
    return json(
      {
        ...
        detail:
          'Set FRISKY_BOT_API_TOKEN (static) or WORKOS_CLIENT_ID + WORKOS_API_KEY (WorkOS) before exposing Fenrir MCP beta.',
      },
      { status: 503 },
      request,
      env
  ...
  return new Response(JSON.stringify({ ok: false, error: 'mcp_auth_required' }, null, 2), {
    status: 401,
    headers,
  });
}
