...
Request JSON must include `community_slug` and `email`. `telegram_user_id` may be included as caller context, but the current Neon schema keys membership by email and does not persist or use a Telegram identity field.
