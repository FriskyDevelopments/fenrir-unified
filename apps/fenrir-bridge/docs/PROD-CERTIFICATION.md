...
3. **MOD 03 subscribe-button payer fix** — callback-initiated invoices previously bound the order to the _bot's_ user id (`query.message.from`), so pre-checkout rejected every real payer. Orders now bind to `query.from` (the human).
