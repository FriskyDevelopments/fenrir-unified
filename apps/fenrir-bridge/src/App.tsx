...
> = {
  en: {
    ...
    neonMagicSuccessMessage:
      "Neon auth link sent. Check your inbox and follow the latest approval step.",
...
export function App() {
  ...
  async function addNewDomain() {
    ...
    if (result.ok) {
      ...
      setNotice(
        `Telegram Lock domain ${result.data.domain} added with tags: ${tags.join(", ")}.`,
      );
      setDomainInput("");
      await refresh();
    }
  ...
  function sendPromisingToWizard() {
    ...
    setDomainTagsInput(addDomainTag(domainTagsInput, "launch"));
    setNotice(
      `${clean.length} clean name${clean.length === 1 ? "" : "s"} queued in the domain wizard. ${clean[0]} is loaded first.`,
    );
  }
  ...
  async function createBridge() {
    ...
    setNotice(
      `Telegram Lock ${result.data.publicUrl} ${ui.ready.toLowerCase()} to share.`,
    );
    await refresh();
  }
      ...
      <main id="main">
        ...
        {show("command") && (
          <LaunchWowConsole
            ...
            onCommunity={() =>
              window.location.assign(communityBridgeDashboardUrl)
            }
          />
        )}
        ...
        <div className="content-grid">
          {show("command", "locks", "telegram") && (
            <section className="panel wide">
              <PanelTitle
                title={c.activeTelegramLocks}
                subtitle={c.activeTelegramLocksSub}
              />
          ...
          {show("command", "billing") && (
            <section className="panel">
              ...
              {billingStatus && (
                <p className="muted">
                  Limits: {billingStatus.limits.maxTelegramLocks ?? "∞"} locks ·
                  custom domain{" "}
              ...
              <div className="pricing">
                {c.plans.map(([plan, price, body]) => (
                  <div className="price" key={plan}>
                    ...
                    <button
                      className="ghost"
                      onClick={() => onPaidPlanPickedFromPricing(plan)}
                    >
                      {c.upgrade}
                    </button>
                  </div>
...
function SessionLabels({
  ...
  const lockLimit =
    backendLimit === null
      ? "unlimited"
      : (backendLimit ?? planLockLimit(state.org.plan));
  const labels = [
    [
      c.sessionRole,
      role === "owner"
  ...
  return (
    ...
    >
      <GlowCard className="auth-card" aria-label="Fenrir Community Gate auth">
        ...
        <div
          className="community-gate-steps"
          aria-label="Community access steps"
        >
          <section>
            <b>01</b>
...
function AuthGate({
  ...
  return (
    <main
      id="main"
      className="lovable-auth-page"
      data-login-source="lovable-bd06c2e4"
    >
        ...
        <section className="lovable-auth-card-wrap" aria-label="Fenrir sign-in">
          ...
          <div className="lovable-auth-card">
            ...
            <div className="lovable-auth-brand">
              ...
              <img
                className="lovable-auth-wordmark"
                src="/fenrir-cut-wordmark.svg"
                alt="MyFenrir wordmark logo"
              />
              <h1>Welcome back</h1>
            ...
            <div className="lovable-auth-actions">
              {(["apple", "google", "microsoft"] as AuthProvider[]).map(
                (provider) => (
                  <AuthProviderButton
                    key={provider}
                    provider={provider}
...
function PublicVaultPage({
  ...
  return (
    <main id="main" className="join-page vault-public-page">
      ...
      {bridges.map((bridge) => {
        ...
        return (
          <article className="lock-card" key={bridge.id}>
            ...
            >
              <span
                className={`status ${bridge.status === "active" ? "good" : "danger"}`}
              >
