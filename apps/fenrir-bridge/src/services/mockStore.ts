...
import type { AppState, AuditAction, FriskyAuditLog, FriskyBridge, FriskyDomain, FriskyLiveRoom, FriskyTelegramInvite, LiveRoomProvider } from "./types";
  ...
  commissionLinks: [
    {
      ...
      commissionNote: "Primary domain partner. Ambassador application pending; paste final affiliate URL here when approved.",
      clicks: 0,
      createdAt: now
    },
    {
      ...
      status: "pending",
      partnerStatus: "pending",
      commissionNote: "Premium/expired domain path. Use for auctions, backorders, and marketplace education.",
      clicks: 0,
      createdAt: now
    },
    {
    ...
    },
    {
      ...
      commissionNote: "Recommended for DNS, SSL, and support reduction. Do not rely on it as cash commission.",
      clicks: 41,
      createdAt: now
    },
    {
    ...
    },
    {
      ...
      commissionNote: "Real referral link added. Use for always-on bot hosting, managed Postgres, and operator deployments; payout is infrastructure credit.",
      clicks: 0,
      createdAt: now
    }
  ]
...
function log(action: AuditAction, targetType: string, targetId: string, metadata: Record<string, string | number | boolean>): FriskyAuditLog {
  return {
...
export function appendAudit(action: AuditAction, targetType: string, targetId: string, metadata: Record<string, string | number | boolean> = {}) {
  store.auditLogs.unshift(log(action, targetType, targetId, metadata));
}

export function trackCommissionClick(linkId: string) {
  const link = store.commissionLinks.find((item) => item.id === linkId || item.url.endsWith(linkId));
  if (!link) {
    return null;
  }
  link.clicks += 1;
  ...
  if (link.provider.toLowerCase().includes("dynadot") && link.url.includes("auction")) {
    appendAudit("domain_auction_opened", "FriskyCommissionLink", link.id, { provider: link.provider });
  }
  return link;
}
