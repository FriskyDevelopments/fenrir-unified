      ...
      ...(env.FENRIR_DEV_MOCK_API === '1' ? [fenrirDevMockApi()] : []),
      {
        ...
        configureServer(server) {
          server.middlewares.use((req, res, next) => {
            ...
            if (!supabaseAuthBase) {
              console.warn(
                `Vite local redirect blocked: VITE_SUPABASE_URL is "${env.VITE_SUPABASE_URL}"`
              );
              res.statusCode = 500;
              res.setHeader('Content-Type', 'text/plain; charset=utf-8');
              res.end(
                'Fenrir auth is misconfigured: VITE_SUPABASE_URL must be the raw Supabase project URL, not localhost or myfenrir.com.'
              );
              return;
            }
...
function normalizeExternalAuthBase(value?: string) {
  ...
  try {
    ...
    if (
      ...
      url.hostname.endsWith('.myfenrir.com') ||
