...
export function OnboardingMotionGuide() {
  ...
  return (
    <section
      className="relative mb-8 overflow-hidden rounded-3xl border border-primary/25 bg-card/75 p-5 shadow-[0_28px_100px_-60px_hsl(var(--primary))] sm:p-6"
      aria-label="Gate setup guide"
    >
      <motion.div
        aria-hidden="true"
        className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-primary/15 blur-3xl"
        animate={{ scale: [1, 1.14, 1], opacity: [0.38, 0.7, 0.38] }}
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold tracking-tight">
            Your gate, from blank to live.
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Follow one clear path. Each step explains exactly what happens next.
          </p>
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                selected
                  ...
                  : "border-border/70 bg-background/30 text-muted-foreground hover:border-primary/30",
              )}
            >
              {selected ? (
                <motion.span
              ...
              <span className="relative flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-background/70 text-primary">
      ...
      >
        ...
        {active === steps.length - 1 ? (
          <Check className="ml-auto h-4 w-4 shrink-0 text-primary" aria-label="Final setup step" />
        ) : null}
      </motion.div>
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-border/60 pt-4">
        <p className="text-xs text-muted-foreground">
          {active === 0
            ? "Start by choosing the gate people will recognize."
            : active === 1
              ? "Telegram is linked only after your gate exists."
              : "Run the member journey before sharing it."}
        </p>
        {active < steps.length - 1 ? (
          <button
            type="button"
          ...
          <Link
            ...
            className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-primary hover:text-primary/80"
          >
            Upgrade options <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        )}
      </div>
    </section>
