...
export function CourtesyPanel({ users }: { users: CourtesyUser[] }) {
    ...
    () =>
      new Map(
        users
          ...
          .map((user) => [String(user.telegram_id), user.email]),
      ),
    [users],
  );
  ...
  return (
    <section className="mb-8 overflow-hidden rounded-2xl border border-violet-400/25 bg-[radial-gradient(circle_at_top_right,rgba(124,58,237,0.16),transparent_42%),rgba(7,10,18,0.78)] p-5 shadow-[0_28px_80px_-55px_rgba(139,92,246,0.9)] sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            Grant temporary Gate access by linked Telegram ID. Group admins cannot see or call these
            controls.
          </p>
        </div>
      ...
      {message ? (
        <p className="mt-4 rounded-lg border border-border/60 bg-background/50 px-3 py-2 text-xs">
          {message}
        </p>
      ) : null}
                ...
                <div className="flex gap-1">
                  {(["30d", "90d", "6m"] as const).map((duration) => (
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      disabled={busy === String(user.telegram_id)}
                      onClick={() => void grantTo(user, duration)}
                    >
                      {duration}
                    </Button>
                  ))}
      ...
      <div className="mt-6 border-t border-border/60 pt-4">
        ...
        {records === null ? (
          ...
          <div className="space-y-2">
            {records.map((record) => (
              <div
                key={record.telegram_user_id}
                className="flex items-center justify-between gap-3 rounded-lg border border-border/50 px-3 py-2 text-sm"
              >
                <div>
                  <span className="font-medium">
                    {byTelegramId.get(record.telegram_user_id) ??
                      `Telegram ${record.telegram_user_id}`}
                  </span>
                  <span className="ml-2 text-xs text-muted-foreground">
                    {record.duration} · expires{" "}
                ...
                <Button
                  size="icon"
                  variant="ghost"
                  disabled={busy === record.telegram_user_id}
                  onClick={() => void revokeFrom(record.telegram_user_id)}
                  aria-label="Revoke courtesy"
                >
