...
create policy "self or staff can read" on public.user_roles for select to authenticated
...
begin
  ...
  if _uid is not null then
    ...
    on conflict (user_id) do update set role = 'owner', telegram_id = 8581086019, updated_at = now();
  end if;
end$$;
...
where r.user_id is null on conflict (user_id) do nothing;
...
returns table (user_id uuid, email text, role public.app_role, telegram_id bigint, created_at timestamptz)
...
declare _caller uuid := auth.uid();
begin
  if _caller is null or not public.is_staff(auth.uid()) then raise exception 'forbidden' using errcode='42501'; end if;
